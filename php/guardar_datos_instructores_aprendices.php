...
if (isset($_POST['nombre_instructor']) && isset($_POST['contacto']) && isset($_POST['nombre_titulada']) && isset($_POST['ambiente'])) {
    ...
    if (!empty($nombreInstructor) && !empty($contactoInstructor) && !empty($titulada) && !empty($ambiente)) {
        // Realizar la inserción del instructor en la tabla de instructores
        $sql = "INSERT INTO instructores (nombre, contacto, titulada, ambiente) VALUES ('$nombreInstructor', '$contactoInstructor', '$titulada', '$ambiente')";
    ...
    if (isset($_POST['instructor']) && isset($_POST['aprendiz']) && $_POST['aprendiz'] !== '') {
        ...
        if (!empty($idInstructor) && !empty($aprendizNombre)) {
            ...
            $sql = "INSERT INTO relacion_instructor_aprendiz (id_instructor, nombre_aprendiz, titulada, ambiente) 
                                 SELECT id_instructor, '$aprendizNombre', titulada, ambiente
                                 FROM instructores 
